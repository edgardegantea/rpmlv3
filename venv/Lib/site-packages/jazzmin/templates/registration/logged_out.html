...
<div class="login-box">
    ...
    <div class="login-box-body">
        ...
        <p class="text-center">{% trans "Gracias por el tiempo que ha dedicado hoy a la administración del sistema." %}</p>
        <p class="text-center mt-5">{% trans "¿Qué desea hacer?" %}</p>

        <div class="row">
            <p class=" col text-center"><a class="btn btn-lg btn-block {{ jazzmin_ui.button_classes.primary }}" href="{% url 'admin:index' %}">{% trans 'Log in again' %}</a>
        </p>

        <p class="text-center col"><a class="btn btn-lg btn-block {{ jazzmin_ui.button_classes.primary }}" href="{% url 'index' %}">{% trans 'Ir al home' %}</a>
        </p>
        </div>
